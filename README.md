...
通过本次实验，我更加深入了解和明确了高速缓存（cache）的工作模式，对存储器层次结构这一章节有了更深刻的理解。

通过这次实验，我更加熟悉了`malloc`函数动态分配内存的使用，也学习了如何使用`getopt`函数来接收运行程序时的选项参数并按这些参数执行不同的功能

这次实验中，我也学会了大致的开发流程：首先构建好框架，再逐一实现大框架下的各种内容以及细节，最后完善部分代码结构。